...
async function readJsonDb() {
  try {
    if (cachedJsonDb) {
      return JSON.parse(JSON.stringify(cachedJsonDb))
    }
    ...
    return { projects: [], leads: [], reviews: [], content: {}, blogs: [], capacities: [], landingPages: [] }
  }
}
...
async function ensurePostgresSchema(client) {
  ...
  try {
    await client.query(`
      ...
      VALUES ('admin@ivr', 'IVRenergy.2026'), ('admin@ivrenergy.com', 'Solar@IVR.2026')
...
export async function readDb() {
  ...
  try {
    ...
    const contentRes = await client.query("SELECT key, value FROM content WHERE key IN ('site_content', 'blogs_data', 'capacities_data', 'landing_pages_data')")
    const contentRow = contentRes.rows.find(r => r.key === 'site_content')
    const blogsRow = contentRes.rows.find(r => r.key === 'blogs_data')
    const capacitiesRow = contentRes.rows.find(r => r.key === 'capacities_data')
    const landingPagesRow = contentRes.rows.find(r => r.key === 'landing_pages_data')
    const content = contentRow?.value || {}
    const blogs = Array.isArray(blogsRow?.value) ? blogsRow.value : []
    const capacities = Array.isArray(capacitiesRow?.value) ? capacitiesRow.value : []
    const landingPages = Array.isArray(landingPagesRow?.value) ? landingPagesRow.value : []
...
export async function writeDb(data) {
  ...
  try {
    ...
    // 1b. Sync blogs (JSONB payload)
    if (Array.isArray(data.blogs)) {
      await client.query(`
        INSERT INTO content (key, value) VALUES ('blogs_data', $1)
        ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value
      `, [JSON.stringify(data.blogs)])
    }

    // 1c. Sync capacities (JSONB payload)
    if (Array.isArray(data.capacities)) {
      await client.query(`
        INSERT INTO content (key, value) VALUES ('capacities_data', $1)
        ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value
      `, [JSON.stringify(data.capacities)])
    }

    // 1d. Sync landingPages (JSONB payload)
    if (Array.isArray(data.landingPages)) {
      await client.query(`
        INSERT INTO content (key, value) VALUES ('landing_pages_data', $1)
        ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value
